...
export const cardClasses = (classes: [string]): string =>
  compose(join(" "), flatten, ifElse(() => classes, append(classes), x => x))([
    "card",
  ]);
...
export const createStyles = (linkAll: boolean): Object =>
  compose(
    ifElse(
      () => linkAll,
      merge({ color: "inherit", textDecoration: "none" }),
      x => x,
    ),
...
export const preloader = (imageClasses: [?string] = []) => (): Object =>
  <div className={`${join(" ")(imageClasses)}`}>
    <div className="ratio__item" />
  </div>;

export const renderActualElement = (
  imageClasses: [?string] = [],
  style: Object,
): Object =>
  <div className={join(" ")(imageClasses)} style={style}>
    <div className="ratio__item" />
  </div>;

// context from ImageLoader
function renderElement() {
  return renderActualElement(this.imageclasses, this.style);
}
...
export const createWrapperClasses = (mobile?: boolean) =>
  compose(
    concat("plain "),
