...
describe("likes list", () => {
  ...
  it("should properly parse category for likes", () => {
    ...
    expect(component.find("MiniCard").get(1).props.category).toEqual(
      "Devotionals",
    );
  ...
  it("should properly parse image for likes", () => {
      ...
      generateComponent({
        likes: [
          ...mockLikes,
          {
            ...
            id: "12345",
            parent: {
              content: { images: [{ fileLabel: "1:1", url: "harambe's dad" }] },
            },
            title: "harambe's dad",
          },
        ],
    ...
    expect(component.find("MiniCard").get(2).props.image).toEqual(
      "harambe-tribute-one",
    );
    expect(component.find("MiniCard").get(3).props.image).toEqual(
      "harambe's dad",
    );
  });
  ...
  it("should properly parse icon for likes", () => {
    ...
    expect(component.find("MiniCard").get(1).props.icon).toEqual(
      "icon-category-text",
    );
  });
  ...
  it("should properly parse link for likes", () => {
    ...
    expect(component.find("MiniCard").get(0).props.link).toEqual(
      "/groups/12345",
    );
    expect(component.find("MiniCard").get(1).props.link).toEqual(
      "/studies/null/entry/12345",
    );
