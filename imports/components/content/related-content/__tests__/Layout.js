...
describe("Related content <Layout>", () => {
  ...
  it("shows a loading state prior to content", () => {
    const tree = renderer.create(<LoadingContent loading={true} />);
    expect(tree).toMatchSnapshot();
  });

  it("has a title", () => {
    const tree = renderer.create(
      <Layout loading={false} title="This is a Title" />,
    );
    expect(tree).toMatchSnapshot();
  });
  ...
  it("renders minicards correctly", () => {
    const content = [
      {
        ...
        content: {
          images: [
            {
              url:
                "//drhztd8q3iayu.cloudfront.net/newspring/editorial/newspring.blog.hero.gingergirl.medium.jpg",
              label: "2:1",
              fileLabel: "2:1",
    ...
    const tree = renderer.create(
      <Layout loading={false} title="This is a Minicard" content={content} />,
    );
    expect(tree).toMatchSnapshot();
  });
});
